...
# Brand forms to deliberately omit from the manifest even though the files exist on disk.
# Keyed by brand slug -> set of forms to skip. The excluded form's files are left untouched
# in the repo; they simply aren't indexed, and the brand falls back to the form it keeps.
EXCLUDE = {
    "brighttalk": {"normal"},   # BrightTALK: endorsed only in the manifest
}
...
def scan_brand(brand_dir: Path) -> dict:
    ...
    for f in sorted(brand_dir.glob("*.png")):
        ...
        if group in excluded:
            continue  # deliberately omitted via EXCLUDE
